...
func TestService_Record(t *testing.T) {
	t.Run("bronze customer earns 1x and saves inside a transaction", func(t *testing.T) {
		...
		customerID := uuid.New()
		customer := customermodel.Customer{ID: customerID, Email: "ana@example.com", Points: 100} // Bronze
	...
	t.Run("tier upgrade triggers both a receipt and a tier-upgrade email", func(t *testing.T) {
		...
		ctx := context.Background()
		customerID := uuid.New()
		customer := customermodel.Customer{ID: customerID, Email: "bruno@example.com", Points: 450} // Bronze, crossing to Silver
	...
	t.Run("email send failure does not fail the request", func(t *testing.T) {
		...
		ctx := context.Background()
		customerID := uuid.New()
		customer := customermodel.Customer{ID: customerID, Email: "ana@example.com", Points: 0}
	...
	t.Run("idempotency returns existing purchase when external_payment_id matches", func(t *testing.T) {
		...
		owner := customermodel.Customer{ID: existing.CustomerID, Points: 50}
	...
	t.Run("race duplicate during transaction is recovered as idempotent success", func(t *testing.T) {
		...
		mocks.Customers.EXPECT().FindByID(ctx, customerID).
			Return(customermodel.Customer{ID: customerID, Points: 0}, nil).Once()
		mocks.expectTxRun(ctx)
		...
		mocks.Customers.EXPECT().FindByID(ctx, customerID).
			Return(customermodel.Customer{ID: customerID, Points: 50}, nil).Once()
	...
	t.Run("OccurredAt is respected when provided", func(t *testing.T) {
		...
		mocks.Customers.EXPECT().FindByID(ctx, customerID).Return(customermodel.Customer{ID: customerID}, nil)
		mocks.expectTxRun(ctx)
