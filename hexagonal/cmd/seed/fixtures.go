...
func buildFixtures() ([]model.Customer, []model.Purchase, []model.Reward) {
	now := time.Now().UTC()
	...
	rewards := []model.Reward{
		...
		newReward(brunoID, model.RewardFreePastry, 75, now.AddDate(0, -1, -15)),
		// Carla redeemed two free drinks
		newReward(carlaID, model.RewardFreeDrink, 100, now.AddDate(0, -3, 0)),
		newReward(carlaID, model.RewardFreeDrink, 100, now.AddDate(0, -1, 0)),
	}

	return customers, purchases, rewards
}

func newPurchase(customerID uuid.UUID, amountCents int64, points int, at time.Time) model.Purchase {
...
func newReward(customerID uuid.UUID, t model.RewardType, cost int, at time.Time) model.Reward {
	return model.Reward{
		ID:          uuid.New(),
		CustomerID:  customerID,
		Type:        t,
